...
# =========================
BRAND_NAME = "Security Pitch"         # ← Your brand
TAGLINE = "SCM Pricing Calculator"    # ← Subtitle shown under brand
PRIMARY = "#0F62FE"                   # ← Brand primary color
ACCENT = "#111827"                    # ← Dark text color
HIGHLIGHT_BG = "#F3F4F6"              # ← Light gray panels
SUCCESS = "#16A34A"
WARNING = "#DC2626"
LOGO_PATH = "logo.png"                # ← Put a logo file next to app.py, or set to None
FAVICON = "🧮"                         # ← Emoji favicon if no logo
...
st.markdown(
    f"""
    <style>
      /* App-wide fonts and spacing */
      .stApp {{ background-color: white; }}
      .app-header {{
        display: flex; align-items: center; gap: 16px; padding: 12px 16px;
        border-radius: 16px; background: linear-gradient(90deg, {PRIMARY}1A, {PRIMARY}0A);
        border: 1px solid #E5E7EB; margin-bottom: 8px;
      }}
      .brand-title {{
        font-weight: 700; font-size: 22px; color: {ACCENT}; margin: 0; line-height: 1.2;
      }}
      .brand-subtitle {{
        font-size: 13px; color: #6B7280; margin: 2px 0 0 0;
      }}
      .panel {{
        background: {HIGHLIGHT_BG}; padding: 16px; border-radius: 14px; border: 1px solid #E5E7EB;
      }}
      .kpi .metric-label {{ color: #6B7280 !important; font-size: 12px !important; }}
      .kpi .metric-value {{ font-size: 22px !important; font-weight: 700 !important; }}
      .footer-note {{ color:#6B7280; font-size:12px; text-align:center; margin-top: 8px; }}
      .contact-sales {{ color:{WARNING}; font-weight:700; font-size:18px; }}
      .ok-green {{ color:{SUCCESS}; font-weight:600; }}
      .badge {{
        display:inline-block; background:{PRIMARY}1A; color:{PRIMARY};
        padding:4px 10px; border-radius:999px; font-size:12px; font-weight:600;
      }}
      .mini-hint {{ color:#6B7280; font-size:12px; }}
...
def thb(n): return f"{int(round(n)):,,d}"

# =========================
# SIDEBAR — MANUAL & BRAND
# =========================
...
if submitted:
    ...
    if r["status"] == "ERROR":
        ...
        st.markdown(f"<div class='panel'><span class='contact-sales'>CONTACT SALES</span><div class='mini-hint'>Totals hidden for projects with more than 100 cameras or any AI tier above 100.</div></div>", unsafe_allow_html=True)
        ...
        st.markdown("<div class='table-note'>Note: MA is informational and not included in Net Total.</div>", unsafe_allow_html=True)
